...
/*   Updated: 2024/08/31 18:30:55 by david            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_redir_handler(t_mini *mini)
{
	...
	{
		if (curr->in_redir[i])
			handle_input_redirection(curr->in_redir[i]);
		if (curr->out_append[i])
			handle_output_append_redirection(curr->out_append[i]);
		if (curr->out_redir[i])
			handle_output_redirection(curr->out_redir[i]);
		i++;
	}
}
